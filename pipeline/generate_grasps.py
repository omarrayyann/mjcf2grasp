...
def _quality_antipodal_worker(batch_data):
    ...
    for p, colliding in zip(transform_batch, collision_batch):
        ...
        l_to_r = (right_contact_point - left_contact_point) / np.linalg.norm(right_contact_point - left_contact_point + 1e-8)
        r_to_l = (left_contact_point - right_contact_point) / np.linalg.norm(left_contact_point - right_contact_point + 1e-8)
...
def in_collision_with_gripper(object_mesh, gripper_transforms, silent=False, num_workers=None):
    ...
    if len(gripper_transforms) < 100 or num_workers <= 1:
        ...
        for tf in tqdm(gripper_transforms, disable=silent):
            min_distance.append(
                np.min([manager.min_distance_single(gripper_mesh, transform=tf) for gripper_mesh in gripper_meshes])
            )
    ...
    for i, (point, normal) in enumerate(zip(points_batch, normals_batch)):
        for roll in rotation_samples:
            ...
            for standoff in standoff_samples:
                ...
                transform = np.dot(
                    np.dot(tra.translation_matrix(origin), trimesh.geometry.align_vectors([0, 0, -1], normal)),
                    orientation,
                )
...
def sample_multiple_grasps(
    ...
    if systematic_sampling:
        ...
        if not silent:
            verboseprint(f"Estimated number of samples: {estimated_candidates:,} ({len(tmp_points):,} points x {len(standoff_samples)} standoffs x {len(rotation_samples)} rotations)")
...
def make_parser():
    ...
    parser.add_argument("--quality", choices=["number_of_contacts", "antipodal"], default="number_of_contacts",
    ...
    parser.add_argument("--systematic_roll_density", type=float, default=15.0,
