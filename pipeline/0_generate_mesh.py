#!/usr/bin/env python3
"""
Script to combine multiple OBJ meshes from a MuJoCo XML file into a single OBJ file.
Applies all transformations (position, rotation, scaling) to create a unified mesh.
"""
...
def parse_mujoco_xml(xml_path):
    """Parse MuJoCo XML and extract mesh information with transformations."""
    ...
    # Parse asset section to get mesh definitions
    meshes = {}
    ...
    # Parse worldbody to get transformations
    mesh_instances = []
    
    def parse_body(body_elem, parent_transform=np.eye(4)):
        """Recursively parse body elements and accumulate transformations."""
        # Get body transformation
        pos = body_elem.get('pos', '0 0 0')
        quat = body_elem.get('quat', '1 0 0 0')  # MuJoCo default: w x y z
        ...
        # Find mesh geometries in this body
        for geom in body_elem.findall('geom'):
            if geom.get('type') == 'mesh':
                mesh_name = geom.get('mesh')
                if mesh_name in meshes:
                    # Get geom-specific transformation
...
def load_and_transform_mesh(mesh_info, xml_dir):
    """Load a mesh file and apply transformations."""
...
def combine_meshes_to_obj(xml_path, output_path, include_visual_only=True):
    """
    Combine all meshes from MuJoCo XML into a single OBJ file.
    
    Args:
        xml_path: Path to MuJoCo XML file
        output_path: Output OBJ file path
        include_visual_only: If True, only include visual geometries (not collision)
    """
