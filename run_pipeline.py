...
for obj in data:
    ...
    try:
        subprocess.run([
            ...
            "--quality", "antipodal",
            "--output", os.path.join(object_output_dir, f"{object_name}_grasps.json"),
            "--systematic_sampling",
    ...
    try:
        ...
        subprocess.run([
            ...
            "--save-png", os.path.join(object_output_dir, f"{object_name}_filtered_grasps_9shot.png"), 
        ...
        #     "--save-png", os.path.join(object_output_dir, f"{object_name}_comparison_9shot.png"), 
        #     "--no-render", "--grasp-shape-only"
        # ], check=True)
    ...
    if os.path.exists(old_grasp_file):
        new_grasp_file = os.path.join(object_output_dir, f"{object_name}_grasps.json")
        os.rename(old_grasp_file, new_grasp_file)
        print(f"Moved {old_grasp_file} to {new_grasp_file}")
    
    if os.path.exists(old_filtered_file):
        new_filtered_file = os.path.join(object_output_dir, f"{object_name}_grasps_filtered.json")
        os.rename(old_filtered_file, new_filtered_file)
        print(f"Moved {old_filtered_file} to {new_filtered_file}")
