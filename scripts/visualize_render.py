...
parser.add_argument(
    ...
    nargs="?",
    default=None,
    help="Object name (required unless --json_file is given)",
)
...
def draw_scene(
    ...
    grasp_scores = grasp_scores[:max_grasps]
    ...
    if render or save_png:
        if save_png:
            ...
            if len(images) == 9:
                ...
                row1 = np.hstack(
                    [resized_images[0], resized_images[1], resized_images[2]]
                )
                row2 = np.hstack(
                    [resized_images[3], resized_images[4], resized_images[5]]
                )
                row3 = np.hstack(
                    [resized_images[6], resized_images[7], resized_images[8]]
                )
                collage = np.vstack([row1, row2, row3])

                # Save high-quality collage
                cv2.imwrite(
                    save_png, collage, [cv2.IMWRITE_PNG_COMPRESSION, 1]
                )  # Minimal compression
                print(
                    f"High-quality 3x3 collage saved to {save_png} (2400x2400 pixels)"
                )
            else:
                print(
                    f"Warning: Could not create all 9 views (got {len(images)}), falling back to single view"
                )
